...
// Set center frequency 865062500 865402500
uint32_t  freq = 865062500; // in Mhz! (868.1)
...
static char description[64] = "IOT LoRaWAN Gateway";                        /* used for free form description */
...
void receivepacket() {
    ...
    if(digitalRead(dio0) == 1)
    {
        if(receivePkt(message)) {
            ...
            FILE *fp;
            fp = fopen("/home/rasp-14841/Desktop/LoRaWAN/packet.txt" , "a+" );
            if(fp == NULL)
            {
                printf("Unable to open file"); printf("\n");
            }
                
            fwrite(message, sizeof(uint8_t), j, fp);
            
            fwrite(message, sizeof(uint8_t), j, stdout);
            fclose(fp);
            printf("\n");
            ...
            //j = b64_to_bin((char *)(buff_up + buff_index), receivedbytes,(uint8_t *)message, 341); 
            buff_index += j;
            buff_up[buff_index] = '"';
            ++buff_index;
