...
typedef struct LoRaWANPacket {
  uint8_t mhdr; // Message Header
  uint32_t DevAddr;
  uint8_t fhdr[7]; // Frame Header
  uint8_t fport; // Port
...
// Decode function
void decodeLoRaWANPacket(uint8_t *msg,LoRaWANPacket decodedPacket) {
  ...
  // Create LoRaWANPacket object

  // Extract fields from the packet
  decodedPacket.mhdr = pack[0];
 // std::copy(pack.begin() + 1, pack.begin() + 4, decodedPacket.DevAddr);
  //std::cout<<"Device Addr : " <<decodedPacket.DevAddr<< std::endl; //<< std::hex 
  
  std::copy(pack.begin() + 1, pack.begin() + 8, decodedPacket.fhdr);
  //std::copy(&decodedPacket.fhdr[0], &decodedPacket.fhdr[4], decodedPacket.DevAddr);
  
  for (int i = 4; i > 0; i--) {
    decodedPacket.DevAddr <<= 8;
    decodedPacket.DevAddr |= (static_cast<uint32_t>(pack[i])); // << (8 *(sizeof(pack) - 1 -i)));
  }
  
  std::cout<<"Device Addr : " << std::hex <<decodedPacket.DevAddr<< std::endl; //<< std::hex 
  decodedPacket.fport = pack[8];
  decodedPacket.payload.assign(pack.begin() + 9, pack.end() - 4);
  decodedPacket.mic = (pack[pack.size() - 4] << 24) |
                      (pack[pack.size() - 3] << 16) |
                      (pack[pack.size() - 2] << 8) |
                      pack[pack.size() - 1];

  // Calculate total length of the packet
  int packetLength = pack.size();

  // Verify the Message Integrity Code (MIC)
  // Replace this logic with your actual MIC verification algorithm
  bool isMicValid = true; // Assume MIC is valid for now

  // Print the decoded fields and other calculated data
  std::cout << "MHDR: " << static_cast<int>(decodedPacket.mhdr) << std::endl;
  std::cout << "FHDR: ";
  for (int i = 0; i < 7; ++i) {
    std::cout <<"fhdr :"<<static_cast<int>(decodedPacket.fhdr[i]) << " "; //<<std::hex 
  }
  std::cout << std::endl;
  ...
  std::cout << "DevAddr: ";
  for (int i = 0; i < 4; ++i) {
    std::cout <<"addr :"<< static_cast<int>(decodedPacket.fhdr[i]) << " ";
  }
  std::cout << std::endl;

  
  std::cout << "FPort: " << static_cast<int>(decodedPacket.fport) << std::endl;
  std::cout << "Payload: ";
  for (const auto& byte : decodedPacket.payload) {
    std::cout <<"pyld "<< static_cast<int>(byte) << " ";
  }
  std::cout << std::endl;
  std::cout << "MIC: " << decodedPacket.mic << std::endl;
  std::cout << "Packet Length: " << packetLength << std::endl;
  std::cout << "MIC Valid: " << (isMicValid ? "Yes" : "No") << std::endl;
}
...
const uint8_t NodeA = 0xaa ;
const uint8_t NodeB = 0xbb ;
const uint8_t NodeC = 0xcc ;

multimap<uint8_t , int> sensorDataMap;
vector<int>myVector;
const int maxCapacity = 5; //Capturing 5 packets
...
uint32_t timertick = 0, loopcnt = 0;
...
void struc_to_array(dwnlnk* data_d, char data[])
{
    for (int i = 0; i < sizeof(data_d); i++)
    {
       data[i] = ((unsigned char*)data_d)[i];
       //printf("data inside : %#x  ",((uint8_t*)&s)[i]);
       //printf("data sta: %#x ",((uint8_t*)data[i]));
       // Serial.print(ans[i]);
    }
    //printf("struc to array : %s ",(data));
}


uint8_t calc_checksum2(packet s) {
    uint8_t checksum = 0;

    for (int i = 0; i < (sizeof(packet)-1); i++)
    {
        checksum += ((unsigned char*)&s)[i];
        //printf("data inside : %c  ",((char*)&s)[i]);
...
uint8_t calc_checksum(void *ptr,int type) {
    ...
    if(type == 0)
    {
        ...
        std::cout << "packet data: "<<(char*)p1->data;
        size = sizeof(packet)-1;
        std::cout << "packet size: "<<size;
        
        for (int i = 0; i < size - 1; i++)
        {
          checksum += ((unsigned char*)p1)[i];
          std::cout << "d : "<<((char*)p1)[i];
    ...
    else
    {
        ...
        std::cout << "Downlink Checksum: "<<(int)checksum;
    }
...
void ISR(uint32_t tick)
{
  if(tick == 0)
  {
      txLoraModem((uint8_t *)"downlink",0x0F,(uint32_t)1890991792,0x07,0x0E,(uint32_t)865062500,0x00,0x40,0);
      cp_up_pkt_fwd++;
  }
}
...
void timerLoop()
{
  ...
  * */
  ...
  txLoraModem((uint8_t *)"downlink",0xF,(uint32_t)1890991792,0x7,0xE,(uint32_t)865062500,0x00,0x27,0);
  cp_up_pkt_fwd++;
}


static void print_curr_sys_time()
{
    time_t t;
...
void timer_callback(union sigval arg)
{
  print_curr_sys_time();
  //uint8_t* data; //[4];
  //printf("Downlink Started \n");
  char data_s[4] ; //= {'0'}; //= "0";
  //data = (uint8_t*)arg.sival_ptr; //- '0';
  //strcpy(data_s,(char*)arg.sival_ptr);
  
  memset(data_s,0x0,4);
  //printf("CB addr: %p\n",(arg.sival_ptr));
  //printf("CB data sival: %#x\n",((char*)arg.sival_ptr));
  
  //for(int i = 0; i<5; i++)
  //{ printf("CB1: %#x ",((uint8_t*)arg.sival_ptr)[i]);}
  
  for(int i = 0; i<4; i++)
  { data_s[i] = ((char*)arg.sival_ptr)[i];} 
  
  //std::cout<<std::endl;
  
  //for(int i = 0; i<5; i++)
  //{ printf("CB2: %#x ",((uint8_t*)data_s[i]));} 
  
  //uint8_t* s = (uint8_t*)arg.sival_ptr;    //puts(s)

  //printf("Sensor data CB hex: %#x\n",data_s);
  printf("Sensor data CB string : %s\n",data_s);
  ...
  //txLoraModem((uint8_t *)"downlink",0xF,(uint32_t)1890991792,0x7,0xE,(uint32_t)865062500,0x00,0x27,0);
  txLoraModem((uint8_t *)data_s,4,tmst,0x7,0xE,(uint32_t)865062500,0x00,0x27,0);
  //delay(500);
  //txLoraModem((uint8_t *)data,sizeof(char),tmst,0x7,0xE,(uint32_t)865062500,0x00,0x27,0);
  //txLoraModem((uint8_t *)s,strlen(s),tmst,0x7,0xE,(uint32_t)865062500,0x00,0x27,0);
  //txLoraModem((uint8_t *)data,strlen(data),1890991792,7,14,865062500,0,0x27,0);
  cp_up_pkt_fwd++;

  printf("Downlink Sent \n");
  //int p = (int) arg.sival_int;  //extract the user data structure
  //printf("Timer Id : ",p);
}

bool timer_tx(char data_p[])
{
    printf("inside timer : %s\n",(data_p));
    //char info[] = "Downlink sent";
    int ret;
    struct sigevent evp;
    ...
    //int i = atoi(data_p);
    //printf("inside timer atoi: %d\n",data_p);
    
    pthread_attr_init(&attr);
    pthread_attr_setstack(&attr,stack,stack_size);
    
    evp.sigev_value.sival_ptr = data_p;
    
    //printf("inside timer addr: %p\n", data_p);
    //printf("inside timer data: %#x\n", data_p);
    
    //for(int i = 0; i<4; i++)
    //{ printf("CB: %#x ",((uint8_t*)data_p[i]));} 
    
    //evp.sigev_value.sival_int = data_p;
    ...
    //create timer, intialised but not fired yet
    ret = timer_create (CLOCK_REALTIME, &evp, &timer); 

    if(ret < 0) { printf("Timer Creation Failed, errno = %d\n",errno); exit(0); }
    ...
    else { print_curr_sys_time(); printf("Timer Alarmed Successfully\n"); return true;}
}

void send_ack(uint8_t addr, dwnlnk *pkt_d)
{
    pkt_d->node_addr = addr;
    pkt_d->data = '0';
    pkt_d->ack = '1';  //means it is ack
          
    uint8_t chksm = calc_checksum(&pkt_d,1);
    pkt_d->chksm = chksm;
    //printf("checkkk : %#x\n",(sensor_d.chksm));
          
    struc_to_array(pkt_d,data_d);
          
    if(timer_tx(data_d)) 
      { 
        printf("Acknowledgement Sent! \n");                         
      }
    else { printf("Acknowledgement Not Sent!\n"); }
      
}

int find_avg_from_map(uint8_t key)
{
    int sum = 0;
    int avg= 0;
        
    // Iterate over the range of elements with the specific key
    ...
    avg = sum / sensorDataMap.count(key);
...
int main()
{
  packet sensor;
  dwnlnk sensor_d;
  int avg_sensor_data = 0;
  
  LoRaWANPacket decodedPacket;
...
ic=0;
  ...
  while(1) {
    ...
    if (ReceivePacket(0)) {
      ...
      // start our Led blink timer, LED as been lit in Receive packet
      led0_timer=millis();
      //printf("%s\n",message);

      decodeLoRaWANPacket(message,decodedPacket);
      
/* Deserialising Capured Packet data */
      printf("message size : %ld \n",strlen((char*)message));
      printf("size : %ld \n",sizeof(sensor));
      
      for(int i = 0; i<strlen((char*)message); i++)
      {
        ((unsigned char *)&sensor)[i] = message[i];
      }

      //uint8_t checksum = calc_checksum2(sensor);
      uint8_t checksum = calc_checksum(&sensor,0);
      printf("CheckSum : %#x\n",checksum);

      if(sensor.chksm == checksum) 
      { printf("CheckSum is same !"); 
        
      int d = atoi(sensor.data);
      
/* Fetching Sensor Data into Map */
      
      switch (sensor.node_addr) 
      {      
        case NodeA:
          {
            sensorDataMap.emplace(NodeA, d);
            sensor_d.node_addr = NodeA;
            size_t keySize1 = sensorDataMap.count(NodeA);
            printf("Data Packets Captured for NodeA : %ld\n",keySize1);
            
            if( sensorDataMap.count(NodeA) != maxCapacity )
            {
              send_ack(NodeA, &sensor_d);
            }
            
          }break;
        case NodeB:
          {
            sensorDataMap.emplace(NodeB, d);
            sensor_d.node_addr = NodeB;
            size_t keySize2 = sensorDataMap.count(NodeB);
            printf("Data Packets Captured for NodeB : %ld\n",keySize2);
            
            if( sensorDataMap.count(NodeB) != maxCapacity )
            {
              send_ack(NodeB, &sensor_d);
            }
        ...
        case NodeC:
          {
            sensorDataMap.emplace(NodeC, d);
            sensor_d.node_addr = NodeC;
            size_t keySize3 = sensorDataMap.count(NodeC);
            printf("Data Packets Captured for NodeC : %d\n",keySize3);
            
            if( sensorDataMap.count(NodeC) != maxCapacity )
            {
              send_ack(NodeC, &sensor_d);
            }
            
          }break;
          
        default:
          { printf("Unidentified Node Data Captured!\n"); }
          break;
      }
...
/* To Print Captured data */
      ...
      if(( sensorDataMap.count(NodeA) == maxCapacity ) || ( sensorDataMap.count(NodeB) == maxCapacity ) || ( sensorDataMap.count(NodeC) == maxCapacity ))
      {
        for (const auto& pair : sensorDataMap) 
        {
          int address = pair.first;
          int data = pair.second;
          printf("Node : %#x , data : %d\n",address,data);
        }       
      }

      
      //checking if Nodes have received n no. of packets
      if(( sensorDataMap.count(NodeA) == maxCapacity ))
      {
          avg_sensor_data = find_avg_from_map(NodeA);
          //printf("Average : %d\n",avg_sensor_data);
          
          sensor_d.node_addr = NodeA;
          if(avg_sensor_data > 160) { sensor_d.data = '0';}
          else { sensor_d.data = '1'; }
          sensor_d.ack = '2';  //means it is action
          
          uint8_t chksm = calc_checksum(&sensor_d,1);
          sensor_d.chksm = chksm;
          //printf("checkkk : %#x\n",(sensor_d.chksm));
          
          struc_to_array(&sensor_d,data_d);
          
          if(timer_tx(data_d)) 
        	{ 
        	  printf("Action Timer Successfully Fired ! \n"); 
            
            auto range = sensorDataMap.equal_range(NodeA);
            if(range.first != range.second)
            { sensorDataMap.erase(range.first); 
              printf("NodeA first data removed!\n"); }
        	  else { printf("NodeA not found!\n"); }
            
        	}
        	else { printf("Action Timer Not Fired! !\n"); }
      }

      else if(( sensorDataMap.count(NodeB) == maxCapacity ))
      {
          avg_sensor_data = find_avg_from_map(NodeB);
          //printf("Average : %d\n",avg_sensor_data);
          
          sensor_d.node_addr = NodeB;
          if(avg_sensor_data > 100) { sensor_d.data = '0';}
          else { sensor_d.data = '1'; }
          sensor_d.ack = '2';  //means it is action
          ...
          if(timer_tx(data_d)) 
            ...
            auto range = sensorDataMap.equal_range(NodeB);
            if(range.first != range.second)
            { sensorDataMap.erase(range.first); 
              printf("NodeB first data removed!\n"); }
        	  else { printf("NodeB not found!\n"); }
            
        	}
        	else { printf("Action Timer Not Fired! !\n"); }
      }
        
      else if(( sensorDataMap.count(NodeC) == maxCapacity ))
      {
          avg_sensor_data = find_avg_from_map(NodeC);
          printf("Average : %d\n",avg_sensor_data);
          
          sensor_d.node_addr = NodeC;
          if(avg_sensor_data > 100) { sensor_d.data = '0';}   //0x30
          else { sensor_d.data = '1'; }                       //0x31
          sensor_d.ack = '2';  //means it is action
          ...
          if(timer_tx(data_d)) 
            ...
            auto range = sensorDataMap.equal_range(NodeC);
            if(range.first != range.second)
            { sensorDataMap.erase(range.first); 
              printf("NodeC first data removed!\n"); }
        	  else { printf("NodeC not found!\n"); }
       ...
       else { printf("CheckSum is not same !\n"); } 
    
    }
/*
      //Fetching Sensor Data into Vector

      if((sensor.node_addr == 0xaa))
      	{
      	  myVector.push_back(d); 
      	  printf("Data Packets Captured : %d\n",(int)myVector.size());
      	}
      
      if( (myVector.size() == maxCapacity) )
      {
        printf("Vec : ");
        for (int i = 0; i < (int)myVector.size(); i++) 
          ...
          //float average = static_cast<float>(sum) / myVector.size();
	
          avg_sensor_data = sum / myVector.size();
          printf("Average : %d\n",avg_sensor_data);
      }

      if((sensor.node_addr == 0xaa) && (myVector.size() == maxCapacity))
      {
        if(timer_tx(avg_sensor_data)) 
	{ 
	  printf("Timer Successfully Fired! \n"); 	  
	  myVector.clear();	
	  if(myVector.empty()) { printf("Vector is empty!\n"); }
	  else { printf("Vector is not empty!\n"); }
	}
	else { printf("Timer Not Fired! !\n"); }
      }
...
/*
      //txLoraModem((uint8_t *)"1",15,1890991792,7,14,865062500,0,64,1);
      //char* data = "downlink";
      
      //txLoraModem((uint8_t *)"downlink",0xF,(uint32_t)(now.tv_sec * 1000000 + now.tv_usec),0x7,0xE,(uint32_t)865062500,0x00,0x40,1);
      //cp_up_pkt_fwd++;
      * 
*/
    }
...
/*
    //pthread_create(&th0, NULL,(void * (*)(void *))timerLoop,NULL);
